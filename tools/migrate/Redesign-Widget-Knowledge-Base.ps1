...
$folders = Get-ChildItem $widgetRoot -Directory | Select-Object -ExpandProperty Name
...
$page = $page -replace 'href="/about/widget-knowledge-base/', 'href="/webact-redesign/about/widget-knowledge-base/'
$page = $page -replace 'href="widget-knowledge-base/', 'href="/webact-redesign/about/widget-knowledge-base/'
$page = $page -replace 'href="\./widget-knowledge-base/', 'href="/webact-redesign/about/widget-knowledge-base/'
$page = $page -replace 'href="\.\./widget-knowledge-base/', 'href="/webact-redesign/about/widget-knowledge-base/'
$page = $page -replace '/webact-redesign/webact-redesign/', '/webact-redesign/'

# Match existing card links to actual folders
foreach ($folder in $folders) {
  $folderNorm = Normalize-Slug $folder
  $title = Title-From-Slug $folder
  $safeFolder = [regex]::Escape($folder)

  # Existing links pointing to .html versions
  $page = $page -replace ('href="[^"]*/' + $safeFolder + '\.html"'), ('href="/webact-redesign/about/widget-knowledge-base/' + $folder + '/index.html"')
  $page = $page -replace ('href="[^"]*/' + $safeFolder + '/?"'), ('href="/webact-redesign/about/widget-knowledge-base/' + $folder + '/index.html"')
}

# Build a clean widget card grid from real folders
$cards = foreach ($folder in $folders) {
  $title = Title-From-Slug $folder
  $desc = "Open this widget guide for setup help, usage tips, and support details for $title."
  $href = "/webact-redesign/about/widget-knowledge-base/$folder/index.html"
...
$css = @'
<style id="widget-kb-redesign-final">
...
.widget-kb-page .eyebrow{letter-spacing:.12em;text-transform:uppercase;font-weight:900;color:#73d7ff}
...
.widget-kb-page .hero-actions{display:flex;flex-wrap:wrap;gap:14px;margin-top:28px}
.widget-final-cta .hero-actions{justify-content:center}
.widget-kb-page .button{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:220px!important;min-height:56px!important;padding:14px 24px!important;border-radius:14px!important;font-weight:900!important;text-decoration:none!important;transition:.2s ease;cursor:pointer}
.widget-kb-page .button.primary,.widget-help-form .button.primary{background:#33abe1!important;border:2px solid #33abe1!important;color:#061421!important}
.widget-kb-page .button.secondary{background:#fff!important;border:2px solid #fff!important;color:#071421!important}
@media(max-width:1080px){
  .widget-kb-hero{grid-template-columns:1fr}
  .widget-kb-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
}
@media(max-width:680px){
  .widget-kb-hero,.widget-section,.widget-final-cta{padding:56px 20px}
  .widget-kb-grid{grid-template-columns:1fr}
  .widget-kb-page .hero-actions .button{width:100%;justify-content:center}
  .widget-kb-proof span{width:100%;text-align:center}
}
</style>
'@

if ($page -notmatch 'widget-kb-redesign-final') {
  $page = $page -replace '</head>', "$css`r`n</head>"
}

$page = $page -replace '<body>', '<body class="widget-kb-page">'
...
<section class="widget-section" id="widget-guides">
  ...
  <div class="widget-kb-grid" data-widget-grid>
    $cardsHtml
  </div>
...
# Replace everything inside main after hero, or create clean main content
if ($page -match '(?s)(<main[^>]*>).*?(</main>)') {
  $page = [regex]::Replace($page, '(?s)(<main[^>]*>).*?(</main>)', "`$1`r`n$newHero`r`n$newMain`r`n`$2", 1)
} else {
  $page = $page -replace '<body class="widget-kb-page">', "<body class=""widget-kb-page"">`r`n<main>$newHero$newMain</main>"
}

$script = @'
<script id="widget-kb-search-script">
(function(){
  ...
  function runSearch(){
    ...
    let shown = 0;
    cards.forEach(card => {
      const match = !q || card.textContent.toLowerCase().includes(q) || (card.getAttribute('data-title') || '').toLowerCase().includes(q);
...
$page = $page -replace '(?s)<script id="widget-kb-search-script">.*?</script>', ''
...
git commit -m "Redesign Widget Knowledge Base and fix search links"
