...
/* WebAct original header compatibility */
document.querySelectorAll("[data-wa-nav]").forEach((header) => {
  const toggle = header.querySelector("[data-wa-menu-toggle], .menu-toggle, .wa-promodo-toggle");
  const menu = header.querySelector("[data-wa-menu], .primary-nav, .wa-promodo-menu");
  const desktop = window.matchMedia("(min-width: 861px)");

  const closeMenus = () => {
    header.querySelectorAll(".wa-promodo-item.is-open, .nav-item.is-open").forEach((item) => {
      item.classList.remove("is-open");
  ...
  header.querySelectorAll(".wa-promodo-item, .nav-item").forEach((item) => {
